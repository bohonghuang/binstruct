    ...
    (with-gensyms (offset)
      ...
      (prog1 (if (global-position-p base)
                           ...
                           (cut
                            (let ((,position (constantly (progn ,position))))
                              ((lambda (,result)
                               ...
                               ,(handler-bind ((partial-byte-error (lambda (c) (declare (ignore c)) (invoke-restart 'ignore))))
        ...
        (unless (symbol-package (car slot))
          (nconcf (cdr *slots*) (list `(nil nil :type (map (null) (constantly ,(car slot)) (constantly null))))))))))
